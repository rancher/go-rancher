...
curl -s http://localhose:8080/v2-beta/schemas?_role=project | jq . > ./input/schemas.json
...
go run *.go -command=generate-docs -version=v1.2 -lang=en -layout=rancher-api-v2-beta-default-v1.2 -apiVersion=v2-beta
echo Success
